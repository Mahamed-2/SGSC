      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {metrics.map((m, i) => (
            <div key={i} className="card p-6 flex items-center justify-between">
              ...
              <div className={cn("p-4 rounded-2xl bg-surface-muted", m.color)}>
        ...
        <div className="card overflow-hidden">
          <div className="p-6 border-b border-surface-border flex items-center justify-between">
            ...
            <span className="flex items-center gap-1.5 text-[10px] font-bold text-danger-DEFAULT bg-danger-light px-2.5 py-1 rounded-full border border-danger-light uppercase tracking-widest">
              <ShieldAlert className="w-3 h-3" /> {t("Privacy Enabled", "الخصوصية مفعلة")}
            </span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left rtl:text-right">
              <thead className="text-xs text-ink-faint uppercase bg-surface-muted">
                <tr>
                  ...
                  <tr key={i} className="hover:bg-surface-subtle transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={cn(
                        ...
                        row.severity === "Severe" ? "bg-danger-light text-danger-DEFAULT border-danger-light" : "bg-brand-50 text-brand-600 border-brand-100"
                      )}>
                        {row.severity}
                      </span>
                    </td>
